...
function Navigation() {
    ...
    const searchState = useSelector( (state) => state.wishlist.search);
  ...
  return (
    <nav className='px-4 bg-white shadow-sm fixed w-full top-0 py-3 flex justify-between items-center z-50'>
       ...
       <input type="text" value={searchState} onChange={(event) => setSearch(event.target.value)}  className="border h-12 flex-1 pl-5" placeholder='Search' />
