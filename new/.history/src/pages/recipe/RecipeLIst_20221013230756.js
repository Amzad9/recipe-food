...
function RecipeList() {
  ...
  const regexp = new RegExp(name.replaceAll(' ', '-'), 'i');
  // const filter = recipe.filter(x => x.title.includes(regexp))
  recipe.filter(function(x) { 
    return x.title.toLowerCase().indexOf(regexp.toLowerCase()) > -1;
  })
  console.log({recipe});
  return (
    <>
