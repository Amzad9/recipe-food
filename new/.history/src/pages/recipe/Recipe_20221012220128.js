  ...
  return (
    ...
    <div className="container mx-auto py-8">
      <div className="grid grid-cols-12 gap-4">
        {recipe.map((item) => (
            <Link to={`/${item.id}`} key={item.id} className="col-span-3 border p-4 rounded-md">
            <img src={item.image} alt={item.title} className="w-full" />
            <h6 className='font-roboto font-light text-gray-600 mt-2'>{item.sourceName}</h6>
            <h4 class="font-medium text-lg mt-3 font-Merienda">{item.title? item.title.slice(1,25)+ "...": item.title}</h4>
            </Link>
        ))}
      </div>
    </div>
        ...
        {recipe.map((item) => (
          item.veryHealthy?
              <RecipeCard headingSection={item["veryHealthy"]} link={`/${item.id}`} image={item.image} imageAlt={item.title} sourceName={item.sourceName} title={item.title} />
             : ""
        ))}
    <div className="container mx-auto py-8">
      <div className="grid grid-cols-12 gap-4">
