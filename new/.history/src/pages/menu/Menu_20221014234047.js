  ...
  return (
    <div className="container mx-auto py-8 mt-32">
      <div className="grid grid-cols-12 gap-4">
        {menu.map((item) => (
            <Link to={`${item.id}`} key={item.id} className="col-span-3 bg-brand-100 p-5 shadow-sm border">
            <img src={item.image} alt={item.title} />
            <h4 className='font-medium text-lg mt-3 font-Merienda capitalize'>{item.title}</h4>
            </Link>
        ))}
        Menu
      </div>
